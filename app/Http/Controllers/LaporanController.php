...
class LaporanController extends Controller
{
    ...
    public function tSetorTarikCetak(Request $request)
    {
        ...
        $data_setor = \App\Setor::whereIn('pesdik_id', $data_id_pesdik)->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->get();
        $total_setor = \App\Setor::whereIn('pesdik_id', $data_id_pesdik)->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->sum('jumlah');

        $data_tarik = \App\Tarik::whereIn('pesdik_id', $data_id_pesdik)->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->get();
        $total_tarik = \App\Tarik::whereIn('pesdik_id', $data_id_pesdik)->whereBetween('created_at', [$tgl_awal, $tgl_akhir])->sum('jumlah');
    ...
    public function tKeuanganSekolahfilterByKategori(Request $request)
    {
        ...
        $data_id_kategori = \App\Pemasukan::select('id')->where('id', $kategori_id)->get();
        $tgl_1 = \App\Pemasukan::first();
        $tgl_awal = $tgl_1->created_at;
        $tgl_akhir = Carbon::now();


        $daftar_kategori = \App\Pemasukan::orderByRaw('kategori_id ASC')->get();
