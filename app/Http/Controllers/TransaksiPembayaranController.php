...
class TransaksiPembayaranController extends Controller
{
    ...
    public function siswaindex($id)
    {
        ...
        return view('/pembayaran/transaksipembayaran/siswaindex', compact('id_pesdik_login', 'data_pesdik', 'tagihan_siswa', 'tagihan_terbayar', 'jumlah_tagihan', 'jumlah_terbayar','pesdik'));
    }
    public function pulangindex($id)
    {
        $pesdik = \App\Pesdik::where('id', $id)->get();
        $id_pesdik_login = $pesdik->first();
        ...
        // $id_tagihan_terbayar = \App\TransaksiPembayaran::select('tagihan_id')->where('pesdik_id', $id)->get();
        $ok = \App\absen::where('id_pesdik', $id) ->whereDate('tanggal', now())->get();
        // dd($tagihan_siswa);
        $tagihan_terbayar = \App\TransaksiPembayaran::where('pesdik_id', $id)
            ->leftJoin('tagihan', function ($join) {
                $join->on('transaksipembayaran.tagihan_id', '=', 'tagihan.id');
        ...
        return view('/pembayaran/transaksipembayaran/pulangindex', compact('id_pesdik_login', 'data_pesdik', 'ok', 'tagihan_terbayar', 'jumlah_tagihan', 'jumlah_terbayar','pesdik'));
    }
    public function rekap($id)
    {
        ...
        // $id_tagihan_terbayar = \App\TransaksiPembayaran::select('tagihan_id')->where('pesdik_id', $id)->get();
        $ok = \App\absen::where('id_pesdik', $id)->get();
        // dd($tagihan_siswa);
        $tagihan_terbayar = \App\TransaksiPembayaran::where('pesdik_id', $id)
            ->leftJoin('tagihan', function ($join) {
                $join->on('transaksipembayaran.tagihan_id', '=', 'tagihan.id');
        ...
        return view('/pembayaran/transaksipembayaran/rekap', compact('id_pesdik_login', 'data_pesdik', 'ok', 'tagihan_terbayar', 'jumlah_tagihan', 'jumlah_terbayar','pesdik'));
    }
    public function updatePublishStatus(Request $request, $id)
    {
        $absen = \App\absen::findOrFail($id);
        ...
        return redirect()->back()->with('warning', 'Status absen sudah diubah sebelumnya.');
    }
    public function updatePublishStatuss(Request $request, $id)
    {
        $absen = \App\absen::findOrFail($id);
        ...
        if ($absen->pulang === null) {
            $absen->pulang = 'pulang';
            $absen->save();
            return redirect()->back()->with('sukses', 'Status absen berhasil diubah menjadi pulang.');
        }
